<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:output doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN"
      doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" indent="yes" method="xml" />
  ...
  <xsl:template match="pmd">
    ...
    <html>
      ...
      <body>
        ...
        <!-- Rules part -->
        <xsl:apply-templates mode="rulelist" select="." />
        <hr align="left" size="1" width="100%" />
  ...
  <xsl:template match="pmd" mode="rulelist">
    ...
    <table border="0" cellpadding="5" cellspacing="2" class="log" width="100%">
      ...
      <xsl:for-each select="file/violation[@rule and generate-id(.) = generate-id(key('violations', @rule))]">
        ...
        <xsl:sort data-type="number" order="descending" select="count(key('violations', @rule))" />

        <xsl:variable name="currentRule" select="@rule" />
        <xsl:variable name="currentSeverity" select="@priority" />
        <xsl:variable name="violationCount" select="count(../../file/violation[@rule=$currentRule])" />
